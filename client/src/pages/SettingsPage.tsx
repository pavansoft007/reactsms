...
const SettingsPage: React.FC = () => {
  ...
  const passwordForm = useForm({
    ...
    validate: {
      new_password: (value: string) => value.length < 6 ? 'Password must be at least 6 characters' : null,
  ...
  const fetchSystemUsers = async () => {
    try {
      ...
      setSystemUsers([
        { id: 1, name: 'Admin User', email: 'admin@school.edu', role: 'Administrator', status: 'active', created_at: '2024-01-01' },
        { id: 2, name: 'John Teacher', email: 'john@school.edu', role: 'Teacher', status: 'active', created_at: '2024-01-15' },
        { id: 3, name: 'Mary Staff', email: 'mary@school.edu', role: 'Staff', status: 'inactive', created_at: '2024-02-01' },
      ]);
    } catch (error) {
                ...
                <form onSubmit={notificationForm.onSubmit(handleNotificationSubmit)}>
                  <Stack>
                    ...
                    <Switch
                      ...
                      {...notificationForm.getInputProps('sms_notifications', { type: 'checkbox' })}
                    />

                    <Switch
                      label="Fee Reminders"
                      description="Send automatic fee payment reminders"
                      {...notificationForm.getInputProps('fee_reminders', { type: 'checkbox' })}
                    />

                    <Switch
                      label="Attendance Alerts"
                      description="Send alerts for attendance issues"
                      {...notificationForm.getInputProps('attendance_alerts', { type: 'checkbox' })}
                    />

                    <Switch
                      label="Exam Notifications"
                      description="Send notifications about upcoming exams"
                      {...notificationForm.getInputProps('exam_notifications', { type: 'checkbox' })}
                    />
            ...
            <Tabs.Panel value="security">
              <Card shadow="sm" p="lg" radius="md" withBorder>
                <form onSubmit={passwordForm.onSubmit(handlePasswordSubmit)}>
                  <Stack>
                    <Alert icon={<IconInfoCircle size={16} />} title="Password Requirements">
                ...
                <Paper shadow="xs" p="md" withBorder>
                  <Stack>
                    {systemUsers.map((user) => (
                      <Group key={user.id} justify="space-between" p="sm" style={{ border: '1px solid #e9ecef', borderRadius: 8 }}>
                        <div>
                          <Text fw={500}>{user.name}</Text>
                          <Text size="sm" c="dimmed">{user.email}</Text>
                          <Group gap="xs" mt="xs">
                            <Badge size="sm" color="blue">{user.role}</Badge>
                            <Badge size="sm" color={user.status === 'active' ? 'green' : 'red'}>
                              {user.status}
                            </Badge>
                          </Group>
                        </div>
                        <Group gap="xs">
                          ...
                          <ActionIcon variant="subtle" color="red" onClick={() => handleDeleteUser(user.id)}>
                            <IconTrash size={16} />
                          </ActionIcon>
                        </Group>
                      </Group>
