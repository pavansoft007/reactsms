...
const StudentsPageUltra = () => {
  ...
  const [sortBy, setSortBy] = useState<"name" | "admission_no" | "class" | "created">("name");
  ...
  const handleCreateStudent = async (values: typeof form.values) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        form.reset();
        fetchStudents(currentPage, searchQuery, selectedClass || "", academicYear?.id?.toString() || "");
      }
    } catch (error: any) {
      notifications.show({
        title: "Error",
  ...
  const handleSearch = (value: string) => {
    ...
    fetchStudents(1, value, selectedClass || "", academicYear?.id?.toString() || "");
  };

  const handleClassFilter = (value: string) => {
    setSelectedClass(value);
    fetchStudents(1, searchQuery, value || "", academicYear?.id?.toString() || "");
  };
    ...
    <SimpleGrid cols={{ base: 2, sm: 4 }} spacing="lg">
      {[
        ...
        <Paper
          ...
          radius="2xl"
          style={{
            background: theme.glassmorphism?.primary || "rgba(255,255,255,0.1)",
        ...
        >
          ...
          <Stack gap="md">
            <Flex justify="space-between" align="flex-start">
              <Box>
                <Text size="xs" c={theme.text.muted} tt="uppercase" fw={700} mb={8}>
                  {stat.label}
                </Text>
      ...
      <Stack gap="lg">
        <Flex gap="lg" wrap="wrap" align="end">
          <TextInput
            ...
            styles={{
              input: {
                background: theme.glassmorphism?.secondary || "rgba(255,255,255,0.05)",
                border: `1px solid ${theme.border}`,
                "&:focus": {
    ...
    >
      ...
      <Stack gap="lg">
        ...
        <Flex align="center" gap="lg" mt="md">
          ...
          <Box style={{ flex: 1 }}>
            <Text fw={700} size="lg" c={theme.text.primary} lineClamp={1} mb={4}>
  ...
  const BulkActionsBar = () => selectedStudents.length > 0 && (
    ...
    >
      <Stack gap="xl">
        ...
        <Group justify="space-between" align="center">
          <Group>
            ...
            <Box>
              ...
              <Text size="sm" c={theme.text.muted}>
                {editingStudent ? "Modify student information" : "Add a new student to the system"}
              </Text>
            </Box>
          </Group>
        ...
        <ScrollArea style={{ height: "60vh" }}>
          <form onSubmit={form.onSubmit(editingStudent ? () => {} : handleCreateStudent)}>
            <Stack gap="2xl">
              {/* Personal Information Section */}
              <Paper p="xl" radius="xl" style={{ background: theme.glassmorphism?.secondary || "rgba(255,255,255,0.05)" }}>
                <Group mb="lg">
        ...
        <Group justify="flex-end" gap="md" pt="lg">
          ...
          <Button
            ...
            onClick={form.onSubmit(editingStudent ? () => {} : handleCreateStudent)}
          >
  ...
  return (
    <Container size="xl" py="xl" style={{ minHeight: "100vh" }}>
      <Stack gap="2xl">
          ...
          {students.length === 0 && !loading ? (
            <Center py="4xl">
              <Stack align="center" gap="xl">
                <ThemeIcon size={120} radius="xl" variant="light" color={theme.colors.primary}>
                  <MdPeople size={60} />
                </ThemeIcon>
        ...
        {totalPages > 1 && (
          <Center>
            <Paper p="md" radius="2xl" style={{ background: theme.glassmorphism?.primary }}>
              <Pagination
                total={totalPages}
                value={currentPage}
                onChange={(page) => {
