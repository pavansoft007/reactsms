...
const BranchesPage: React.FC = () => {
  ...
  const form = useForm({
    ...
    validate: {
      name: (value: string) =>
        value.trim().length === 0 ? "Branch name is required" : null,
      school_name: (value: string) =>
        value.trim().length === 0 ? "School name is required" : null,
      email: (value: string) =>
        /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value) ? null : "Invalid email",
      mobileno: (value: string) =>
        value.trim().length === 0 ? "Mobile number is required" : null,
      currency: (value: string) =>
        value.trim().length === 0 ? "Currency is required" : null,
      symbol: (value: string) =>
        value.trim().length === 0 ? "Currency symbol is required" : null,
    },
  });
